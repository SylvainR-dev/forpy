...
def build_qcm_architecture_screen(page: ft.Page, session: SessionState) -> ft.View:
    ...
    view = ft.View(
        route="/qcm_architecture",
        bgcolor=c["bg"],
        controls=[
            ft.AppBar(
                title=ft.Text(t.get("qcm_architecture", "Architecture Quiz"), color=c["text"]),
                bgcolor=c["bg"],
...
async def _async_generate(
    ...
    try:
        ...
        prompt = _builder.get_prompt(
            ...
            sublevel="qcm_architecture",
            exercise_language=session.exercise_language,
            last_topic=session.last_exercise_topic,
        )

        provider = get_provider(session.provider)
        result = await provider.call_async(prompt, session.api_key, "qcm_architecture")

        questions = result.get("_qcm_list", [])
        if not questions:
            raise ValueError("json")

        session.update_last_topic("qcm_architecture")
